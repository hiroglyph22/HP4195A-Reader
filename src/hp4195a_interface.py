...
class HP4195AInterface(multiprocessing.Process):
    ...
    def _handle_get_machine_values(self) -> None:
        """Handle request to get all machine configuration values."""
        self.logger.info('Querying machine configuration values')
        
        if not self.instrument:
            self.logger.error('Cannot query machine values: not connected to instrument')
            self.message_queue.put(False)
            return
        ...
        try:
            # Query all machine status values
            queries = [
                ('device_id', GPIBCommands.QUERY_IDENTITY.value),
                ('center_frequency', GPIBCommands.QUERY_CENTER.value),
                ('span', GPIBCommands.QUERY_SPAN.value),
                ('start_frequency', GPIBCommands.QUERY_START.value),
                ('stop_frequency', GPIBCommands.QUERY_STOP.value),
                ('resolution_bandwidth', GPIBCommands.QUERY_RBW.value),
                ('oscillator_1_amplitude', GPIBCommands.QUERY_OSC1.value)
            ]
            
            for key, query_cmd in queries:
                try:
                    response = self.send_query(query_cmd)
                    if response and response != "Query failed" and response != "Not connected":
                        # Clean up response for device_id 
                        if key == 'device_id':
                            machine_values[key] = response.splitlines()[0] if response else "Unknown"
                        else:
                            # Try to parse numeric values
                            try:
                                # Remove units and convert to float
                                clean_value = response.replace('HZ', '').replace('DBM', '').strip()
                                machine_values[key] = float(clean_value)
                            except (ValueError, AttributeError):
                                machine_values[key] = response
                    else:
                        machine_values[key] = "Query failed"
                        
                except Exception as e:
                    self.logger.warning(f'Failed to query {key}: {e}')
                    machine_values[key] = "Error"
                    
            # Add connection status
            machine_values['connection_status'] = 'Connected'
            machine_values['sweep_mode'] = 'Single'  # Default assumption
            
            self.logger.info(f'Successfully queried machine values: {machine_values}')
            self.message_queue.put(True)
            self.data_queue.put(machine_values)
